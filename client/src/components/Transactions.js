    ...
    <div>
      <CTable align="middle" className="mb-0 border" hover responsive>
        <CTableHead color="dark">
          <CTableRow>
            <CTableHeaderCell className="text-center" onClick={() => applySorting("type", !sorting.ascending)}>Type</CTableHeaderCell>
            <CTableHeaderCell className="text-center" onClick={() => applySorting("sender", !sorting.ascending)}>Sender</CTableHeaderCell>
            <CTableHeaderCell className="text-center" onClick={() => applySorting("receiver", !sorting.ascending)}>Receiver</CTableHeaderCell>
            <CTableHeaderCell className="text-center" onClick={() => applySorting("ammount", !sorting.ascending)}>Ammount</CTableHeaderCell>
            <CTableHeaderCell className="text-center" onClick={() => applySorting("status", !sorting.ascending)}>Status</CTableHeaderCell>
          </CTableRow>
        </CTableHead>
        <CTableBody>
          {transactions.map((item, index) => (
            <CTableRow v-for="item in tableItems" key={index}>
              ...
              <CTableDataCell className="text-center">
                {item.status == "approved"? <MdCheckCircle color='green'/> : <MdDangerous color='red'/>}
              </CTableDataCell>
            </CTableRow>
          ))}
        </CTableBody>
